...
tiny sum(tiny a, tiny b) {
  ...
  unsigned int exponent_difference = exponent_a > exponent_b
  ...
  if (sign_a || sign_b) {
    ...
    sign_r = sign_a == 0 ? significand_b > significand_a
                         : significand_a > significand_b;
  } else {
    significand_r = significand_a + significand_b;
    sign_r = sign_a;
  ...
  int fix_down =
      exponent_difference + (significand_a + significand_b > 0x07) - 1;
  int round = 0;
...
// possible; however they are good enough for testing purposes. note: they also
// do not cast to zero properly
...
int main() {
  // hmm, its difficult to parse out what is just rounding errors, and what is

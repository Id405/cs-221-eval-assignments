...
tiny mult(tiny a, tiny b) {
  ...
  unsigned int significand_b =
      (b & 0x07) | 0x08; // currently with this method, three bits of potential
                         // adding space are wasted
  ...
  unsigned char exponent_r =
      (char)(((char)exponent_a - 7) + ((char)exponent_b - 7) + 7);
  ...
  if (fix_down > 0 && (significand_r >> (fix_down - 1)) & 1) {
    int mask = ((2 << (fix_down - 2)) -
                1);             // mask all bits below deciding bit for rounding
...
// these are not perfect casting functions achieving the highest accuracy
// possible; however they are good enough for testing purposes.
tiny cast_to_tiny(float n) { // TODO: rounding, are these functions converting
                             // with implicit 1 incorrect?
  int n_int = *(int *)&n;
  char sign = (n_int >> 31);
...
float cast_from_tiny(tiny n) {
  ...
  int result = ((((sign & 0x01) << 31) | ((exponent + 120) << 23) |
                 (significand << 20)));
  return *(float *)&result;
}
